...
permalink: /GameDetails/Map/MapTilesAndTilesets
---
...
In order to understand how the [map file]() is structured/read in, the concept of a "map tile" must be understood first.
...
which details on can be found [here](). The general idea is that the class `MapTileBuilder`, which can be found in the `Builders`
...
the builder pattern (you can read more about it [here]()), ideally you can use the `CommonTileset` class as a reference to guide you
...
To start, a new `Frame` (details on `Frame` class [here]()) must be created to represent the grass tile's graphic (`grassFrame`. The `FrameBuilder`
